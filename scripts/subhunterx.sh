...
tools=(amass subfinder findomain assetfinder sublist3r crobat httpx nmap whatweb ffuf waybackurls gau paramspider nuclei nikto sqlmap xsstrike commix gospider subjs crtsh naabu katana chaos shuffledns masscan pandoc linkfinder dalfox arjun)
...
amass enum -passive -silent -config "$config_file" -d "$domain" -o "$output_dir/amass.txt" & 
subfinder -silent -d "$domain" -o "$output_dir/subfinder.txt" & 
...
crobat -s "$domain" | tee -a "$output_dir/crobat.txt" & 
chaos -d "$domain" -o "$output_dir/chaos.txt" & 
wait

# CRT.sh Subdomain Discovery with error handling
echo -e "${BLUECOLOR}[+] Discovering subdomains from CRT.sh...${RESETCOLOR}"
if ! python3 crtsh.py -d "$domain" | tee -a "$output_dir/crtsh.txt"; then
    echo "Warning: CRT.sh enumeration failed"
fi
